...
const PESSOAS = ['Pessoa 1', 'Esposa', 'Casal']

export default function Lancamentos({ mes, ano }) {
  const { loading, lancamentos, config, despesasFixas, despesasVariaveis, investimentos, totalDespesas, totalCartao, saldoMes, receitaTotal, adicionarLancamento, excluirLancamento } = useFinanceiro(mes, ano)
  ...
  const salvar = async () => {
    ...
    setSaving(false)
    setModal(false)
    setForm({ data: '', tipo: 'Despesa Fixa', pessoa: '', categoria: CATEGORIAS[0], valor: '', forma_pagamento: 'Débito', descricao: '', observacao: '' })
  }
            ...
            <tbody>
              {filtrados.map(l => (
                <tr key={l.id}>
                  ...
                  <td>
                    <button className="btn btn-danger btn-icon btn-sm" onClick={() => excluirLancamento(l.id)}>
                      <Trash2 size={13} />
                    </button>
                  </td>
                </tr>
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <div className="form-group">
                ...
                <input value={form.descricao} onChange={e => setForm(f => ({ ...f, descricao: e.target.value }))} placeholder="Ex: Aluguel, Netflix, Mercado..." />
              </div>
              <div className="form-group">
